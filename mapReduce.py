...
class OrderDateCount(MRJob):
    ...
    def sort(self, key, values):
        data = []
        for order_total, order_count in values:
            data.append((order_total, order_count))
            data.sort()

        for order_total, order_count in data:
           yield order_total, order_count
